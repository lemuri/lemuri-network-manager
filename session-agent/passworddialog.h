...
public:
    explicit PasswordDialog(const NMVariantMapMap &connection,
                            ...
                            QWidget *parent = 0);
    ~PasswordDialog();
    void setSecretsMap(const NMStringMap &secrets);
    void setupGenericUi(const NetworkManager::ConnectionSettings::Ptr connectionSettings);
    void setupVpnUi(const NetworkManager::ConnectionSettings::Ptr connectionSettings);
