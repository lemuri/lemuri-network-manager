...
void PasswordDialog::setSecretsMap(const NMStringMap &secrets)
{
    Q_ASSERT(ui);
    if (!secrets.isEmpty()) {
        ui->password->setText(secrets.value(QLatin1String("psk")));
        ui->password->selectAll();
    }
}

void PasswordDialog::setupGenericUi(const ConnectionSettings::Ptr connectionSettings)
{
    ...
    ui->labelIcon->setPixmap(QIcon::fromTheme("dialog-password").pixmap(32));
    ...
    if (wifi && (connectionType == Setting::WirelessSecurity || connectionType == Setting::Security8021x)) {
        ...
        ui->labelText->setText(QString("For accessing the wireless network '%1' you need to provide a password below").arg(ssid));
    } else {
        ui->labelText->setText(tr("Please provide a password below"));
    }
...
void PasswordDialog::setupVpnUi(const ConnectionSettings::Ptr connectionSettings)
{
...
{
    ui->password->setEchoMode(show ? QLineEdit::Normal : QLineEdit::Password);
}
